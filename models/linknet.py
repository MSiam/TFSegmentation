...
class LinkNET(BasicModel):
    ...
    def init_network(self):
        ...
        with tf.variable_scope('output_block'):
            self.out_full_conv1 = self._deconv('deconv_out_1', self.out_decoder_block_1, 32, (3, 3))
            ...
            self.fscore = self._deconv('deconv_out_2', self.out_conv1, self.params.num_classes, (2, 2))
    ...
    def _full_conv_3x3_block(self, name, x, out_channels):
        with tf.variable_scope(name):
            out = self._deconv('deconv', x, out_channels, kernel_size=(3, 3))
            out = tf.layers.batch_normalization(out, training=self.is_training, fused=True)
            out = tf.nn.relu(out)
        return out

    def _deconv(self, name, x, out_channels, kernel_size=(3, 3)):
        with tf.variable_scope(name):
