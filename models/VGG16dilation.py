...
class VGG16Dilation(BasicModel):
    ...
    def init_network(self):
        ...
        with tf.name_scope('dilation_2'):
            ...
            self.fc6_dil = atrous_conv2d('fc6_dil', x=self.encoder.conv5_3_dil, num_filters=1024,
                                             kernel_size=(7, 7), dilation_rate=4, activation=tf.nn.relu,
                                             l2_strength=self.encoder.wd, dropout= 0.5, is_training= self.is_training)

            self.fc7 = load_dense_layer(self.reduced_flag, self.fc6_dil, 'fc7', self.pretrained_weights,
                                    activation=tf.nn.relu, dropout=0.5,
                                    train=self.is_training, l2_strength=self.wd)

            self.score_fr = load_dense_layer(self.reduced_flag, self.fc7, 'score_fr', self.pretrained_weights,
                                         num_classes=self.params.num_classes, l2_strength=self.wd)
