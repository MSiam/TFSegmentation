...
class Onavos_2stream(Onavos):
    def __init__(self, sess, config):
        super(Onavos_2stream, self).__init__(sess, config)
        self.config = config
        self.sess = sess
...
#       tf.summary.scalar('valid_iou', self.test_net['measures']['iou'])
        ...
        self.merged_summs= tf.summary.merge_all()
    ...
    def prepare_datasets(self):
        if self.config.task == "train":
            ...
            tf.summary.image('train_img', self.train_inputs)
            self.train_flow = inputs_tensors_dict["flow"]
            tf.summary.image('train_flow', self.train_flow)
            self.train_labels = inputs_tensors_dict["labels"]
            tf.summary.image('train_label', self.train_labels)
        ...
        self.valid_data = load_dataset(self.config, "valid", self.sess, self.coordinator, of_flag= True)
        inputs_tensors_dict = self.valid_data.create_input_tensors_dict(self.config.batch_size, of_flag=True)
        self.valid_inputs = inputs_tensors_dict["inputs"]
        tf.summary.image('valid_img', self.valid_inputs)
        self.valid_flow = inputs_tensors_dict["flow"]
        tf.summary.image('valid_flow', self.valid_flow)
        self.valid_labels = inputs_tensors_dict["labels"]
        tf.summary.image('valid_labels', self.valid_labels)
    ...
    # model
    def create_stream(self, name, reuse, inputs):
    ...
    def build(self, inputs,flow, labels,reuse):
        with tf.variable_scope("",reuse=reuse):
            ...
            out, pred, measures, loss = self.segmentation_softmax('output', conv1, labels, 2, None, filter_size=(3, 3),
    ...
    def parse_onavos_weights(self, model_path):
        with open(model_path, 'rb') as output:
            ...
            model_names, model_names_raw = zip(*sorted(zip(model_names, model_names_raw)))
            ...
            for i in range(len(loaded_model_names)):
                ...
                assign_op = model_weights[model_names_raw[i]].assign(loaded_weights[loaded_model_names[i]])
                print('set weight in model ', name)
    ...
    def one_shot_evaluation(self):
        ...
        for idx in video_ids:
            ...
            for i in tqdm(range(n_total)):
                ...
                feed_dict = self.valid_data.feed_dict_for_video_frame(frame_idx=i, with_annotations=True, of_flag=True)
