...
class UNetMobileNet(BasicModel):
    ...
    def init_network(self):
        ...
        with tf.name_scope('upscale_1'):
            self.expand11 = conv2d('expand1_1', x=self.encoder.conv5_6, batchnorm_enabled=True,
                                      num_filters=self.encoder.conv5_5.shape.as_list()[3], kernel_size=(1, 1),
                                      l2_strength=self.encoder.wd)
            self._debug(self.expand11)
            self.upscale1 = conv2d_transpose('upscale1', x=self.expand11,
            ...
            self.expand12 = conv2d('expand1_2', x=self.add1, batchnorm_enabled=True,
                                      num_filters=self.encoder.conv5_5.shape.as_list()[3], kernel_size=(1, 1),
                                      l2_strength=self.encoder.wd)
            self._debug(self.expand12)

        with tf.name_scope('upscale_2'):
            self.expand21 = conv2d('expand2_1', x=self.expand12, batchnorm_enabled=True,
                                      num_filters=self.encoder.conv4_1.shape.as_list()[3], kernel_size=(1, 1),
                                      l2_strength=self.encoder.wd)
            self._debug(self.expand21)
            self.upscale2 = conv2d_transpose('upscale2', x=self.expand21,
            ...
            self.expand22 = conv2d('expand2_2', x=self.add2, batchnorm_enabled=True,
                              num_filters=self.encoder.conv4_1.shape.as_list()[3], kernel_size=(1, 1),
                              l2_strength=self.encoder.wd)
            self._debug(self.expand22)

        with tf.name_scope('upscale_3'):
            self.expand31 = conv2d('expand3_1', x=self.expand22, batchnorm_enabled=True,
            ...
            self.expand32 = conv2d('expand3_2', x=self.add3, batchnorm_enabled=True,
                                num_filters=self.encoder.conv3_1.shape.as_list()[3], kernel_size=(1, 1),
                                l2_strength=self.encoder.wd)
            self._debug(self.expand32)

        with tf.name_scope('upscale_4'):
            self.expand41 = conv2d('expand4_1', x=self.expand32, batchnorm_enabled=True,
            ...
            self.expand42 = conv2d('expand4_2', x=self.add4, batchnorm_enabled=True,
                                      num_filters=self.encoder.conv2_1.shape.as_list()[3], kernel_size=(1, 1),
                                      l2_strength=self.encoder.wd)
        ...
        with tf.name_scope('upscale_5'):
            ...
            self.expand5 = conv2d('expand5', x=self.upscale5, batchnorm_enabled=True,
                                      num_filters=self.encoder.conv1_1.shape.as_list()[3], kernel_size=(1, 1),
                                      l2_strength=self.encoder.wd)
            self._debug(self.expand5)

        with tf.name_scope('final_score'):
            self.fscore = conv2d('fscore', x=self.expand5, batchnorm_enabled=True,
