...
class DAVISOFDataset(ImageDataset):
  def __init__(self, config, subset, coord, fraction=1.0):
    super(DAVISOFDataset, self).__init__("davis", DAVIS_DEFAULT_PATH, NUM_CLASSES, config, subset, coord,
                                       DAVIS_IMAGE_SIZE, VOID_LABEL, fraction, lambda x: x / 255, ignore_classes=[VOID_LABEL])
    self.trainsplit = config.trainsplit
    self.of_flag= True
