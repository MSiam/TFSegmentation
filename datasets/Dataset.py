...
class Dataset(object):
  ...
  def _get_resize_params(self, subset, input_size_default, resize_mode_default=ResizeMode.Unchanged):
    if subset == "valid":
      subset = "val"
      resize_mode_string = self.config.resize_mode_val

    elif subset=="train":
      resize_mode_string = self.config.resize_mode_train
    ...
    input_size = self.config.input_size_val
    if input_size==None or len(input_size) == 0:
