from datasets.DAVIS.DAVIS import DAVISDataset, DAVIS2017Dataset
from datasets.DAVIS.DAVIS_of import DAVISOFDataset
...
from datasets.DAVIS.DAVIS_oneshot_of import DavisOFOneshotDataset
...
def load_dataset(config, subset, session, coordinator, of_flag=False):
  name = config.dataset.lower()
  task = config.task
  if of_flag:
    if task in ("oneshot", "oneshot_forward", "online", "offline"):
        if name == "davis":
          return DavisOFOneshotDataset(config, subset, use_old_label=False)
        else:
          assert False, "Unknown dataset for oneshot: " + name
    if name == "davis":
        return DAVISOFDataset(config, subset, coordinator)
  else:
